...
def generate_article(existing_titles):
    styles = [
        ...
        "une analyse tactique d'un match ou d'un système de jeu récent",
...
RÈGLES STRICTES :
1. Base-toi UNIQUEMENT sur des faits réels et récents trouvés via ta recherche Google. Cite des événements, scores, noms de joueurs réels de la saison 2025/2026.
2. NE génère PAS un article sur un sujet déjà traité :
{titles_str}
3. Si le sujet demandé est déjà traité, choisis un angle totalement différent.
4. Ne mentionne jamais que l'article est généré par une IA.
...
{{
  ...
  "unsplash_query": "requête anglais pour trouver une image football générique (ex: football stadium crowd, soccer match action, porto stadium night)",
  "content": "HTML complet : minimum 5 balises <p> avec contenu riche, 2 balises <h2> comme sous-titres. Style journalistique passionné. PAS de balises html/head/body/article."
}}"""
...
<head>
  ...
  <style>
    ...
    *,*::before,*::after{{margin:0;padding:0;box-sizing:border-box;}}
    ...
    .nav-cta{{font-family:'Barlow Condensed',sans-serif;font-size:11px;font-weight:700;letter-spacing:2px;text-transform:uppercase;border:1px solid rgba(255,255,255,0.18);color:#fff;background:transparent;padding:8px 22px;text-decoration:none;transition:background .2s;}}
    ...
    .article-body h3{{font-family:'Barlow Condensed',sans-serif;font-size:18px;font-weight:700;text-transform:uppercase;letter-spacing:1px;margin:32px 0 12px;color:rgba(255,255,255,0.85);}}
  ...
  <div class="article-hero">
    ...
    <div class="article-hero-content">
      ...
      <div class="article-meta-bar">
        <span>{date_str}</span><span>·</span><span>{data['read_time']}</span>
      </div>
    </div>
  </div>
...
def main():
    ...
    print("🔍 Génération Gemini + Google Search...")
    ...
    index["articles"].insert(0, {
        ...
        "image":       img_url,
        "image_thumb": image["thumb"] if image else img_url
    })
    if image:
        index["used_image_ids"].append(image["id"])

    save_index(index)
    print(f"✅ Sauvegardé : articles/{filename} — Index : {len(index['articles'])} articles")
