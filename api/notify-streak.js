// api/notify-streak.js — Rappel streak : "Tu vas perdre ton streak !"
// Appelé par un cron Vercel ou manuellement
// Body: { toUserId, streak, hoursLeft }
...
function buildEmail({ toName, streak, hoursLeft }) {
  const days = Array.from({length:7}, (_,i) => {
    ...
    return `<td style="padding:4px"><div style="width:34px;height:34px;background:${bg};border:${border};display:flex;align-items:center;justify-content:center;font-family:Helvetica,Arial,sans-serif;font-size:10px;font-weight:700;color:${color};text-align:center;line-height:34px">${label}</div></td>`;
  }).join('');
...
<head>
  ...
  <style>
    ...
    @media(max-width:600px){.wrap{padding:0!important}.box{border:none!important}.inner{padding:24px 16px!important}.head{padding:16px!important}.foot{padding:16px!important}.hero{font-size:22px!important}.days td{padding:2px!important}.day-box{width:28px!important;height:28px!important;font-size:9px!important;line-height:28px!important}}
  </style>
</head>
    ...
    <td class="head" style="padding:20px 28px;border-bottom:1px solid #f0f0f0">
      <table role="presentation" cellpadding="0" cellspacing="0"><tr>
        ...
        <td style="vertical-align:middle">
          ...
          <span style="font-family:Helvetica,Arial,sans-serif;font-size:9px;letter-spacing:2px;text-transform:uppercase;color:#999">Rappel de connexion</span>
        </td>
      </tr></table>
    </td>
  </tr>
      ...
      <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background:#000">
        ...
        <tr><td class="inner" style="padding:28px">
          <table role="presentation" cellpadding="0" cellspacing="0" style="margin-bottom:14px"><tr>
            <td style="width:18px;height:2px;background:#f0a500;padding-right:10px;vertical-align:middle;font-size:0">&nbsp;</td>
            <td style="font-family:Helvetica,Arial,sans-serif;font-size:9px;font-weight:700;letter-spacing:4px;text-transform:uppercase;color:rgba(255,255,255,0.4)">Streak · ${streak} jours</td>
          </tr></table>
          ...
          <table role="presentation" cellpadding="0" cellspacing="0"><tr>
            <td style="border:1px solid rgba(255,255,255,0.5)">
              <a href="https://newsporto.fr" style="display:inline-block;font-family:Helvetica,Arial,sans-serif;font-size:10px;font-weight:700;letter-spacing:3px;text-transform:uppercase;color:#fff;text-decoration:none;padding:12px 28px">ME CONNECTER &nbsp;→</a>
            </td>
          </tr></table>
        </td></tr>
      </table>
      ...
      <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="border:1px solid #eee;border-top:none"><tr>
        <td style="padding:14px 20px">
          <table role="presentation" cellpadding="0" cellspacing="0"><tr>
            ...
            <td>
              <span style="font-family:Helvetica,Arial,sans-serif;font-size:11px;font-weight:700;letter-spacing:1px;text-transform:uppercase;color:#111;display:block">${streak} jours de suite</span>
              <span style="font-family:Helvetica,Arial,sans-serif;font-size:11px;color:#888">Ne perds pas ta progression · Connecte-toi avant minuit</span>
            </td>
          </tr></table>
        </td>
      </tr></table>
...
export default async function handler(req, res) {
  ...
  const { toUserId, streak, hoursLeft = 6 } = req.body || {};
  if (!toUserId || !streak) return res.status(400).json({ error: 'Missing fields' });
  ...
  try {
    ...
    await resend.emails.send({
      ...
      html: buildEmail({ toName, streak, hoursLeft }),
    });
    return res.status(200).json({ ok: true });
  } catch (err) {
    console.error('notify-streak error:', err);
